...
## Free tools (real, but running on heuristics in production)
...
Mega menu lists all eight, from the CMS (`withCmsNav`), so a tool added in
WordPress appears there on its own.

**They are all falling back to heuristics on the live site.** The footnote
gives it away: "Scored with our copy heuristics." rather than "Scored by AI
with a conversion-copy rubric." `GEMINI_API_KEY` reaches the Node process —
`SetEnv GEMINI_API_KEY` in `~/public_html/.htaccess`, proven with a value-free
probe that reported the name present in a production `process.env` — so the
hosting mechanism is correct and the credential is what is being rejected. The
key worked on Vercel on 2026-07-18 and is 53 characters beginning `AQ.`, not
the 39-character `AIza` form AI Studio issues, which is the short-lived kind.
**Fix: a fresh key from aistudio.google.com, replacing the value on that same
`SetEnv` line and in local `.env`.** Nothing in the code needs to change.

Every tool degrades silently by design, which is why nobody noticed. That is
right for visitors and means the only way to know is to read a footnote or
check `ai: true` in a response.
...
All of it is emitted once from `__root.tsx` and applies to every route,
including the CMS-driven ones that have no route file to edit.

- **Canonical + `og:url`** come from `canonicalPath(matches)` in
  `src/lib/seo.ts`. Search params are dropped on purpose: the header links to
  `/blog?cat=…` four times, so the blog existed at five URLs with identical
  content and nothing said which was the address.
- **Share card** `public/og.jpg`, 1200x630, generated by `scripts/og-card.html`
  from the site's own tokens — it asks a browser to resolve `oklch(...)` and
  the webfont rather than approximating them. The root declared
  `twitter:card = summary_large_image` from the first commit and never declared
  an image, so every link to the site rendered a blank card. **It is
  placeholder-grade; a designed card would be better.**
- **Per-page images**: articles and case studies share their own picture via
  `shareImageMeta` in their own `head()`. It has to be per-route — the root's
  `head()` runs before child loaders resolve, so `matches[last].loaderData` is
  empty there (measured: `keys=[]`). That also rules out breadcrumbs from the
  root, which is why there are none. The helper ignores picsum and friends, so
  a post with no cover falls back to the brand card instead of broadcasting a
  random stock photo.
- **JSON-LD**: Organization + WebSite + WebPage as a `@graph` with `@id`
  references. **No `contactPoint`** — the phone number in the CMS is in the
  reserved-for-fiction 555-01xx range, and structured data is machine-read and
  cached, so a placeholder there outlives the page. Add one when the details
  are real.
- **robots.txt + sitemap.xml** are generated at build time by
  `scripts/gen-sitemap.mjs`, 70 URLs. The `learning` collection is deliberately
  not in it: its 22 docs are list entries on the four coded category pages, not
  pages of their own, and listing them advertised 22 addresses no route has
  ever matched.
- **Missing content is a 404, not a 200.** All eight `$slug` routes throw
  `notFound()`. The condition differs per route — a catalog page exists if
  *either* the CMS or the built-in set has the slug, so the throw checks both.
  `/learning` keeps its own contextual not-found page via `notFoundComponent`.
- **Accessibility**: a skip link is the first focusable on every page (the mega
  menu is ten tab stops), `<main>` carries `tabIndex={-1}` so activating it
  moves focus rather than only scrolling, and its z-index clears the header's
  9999. Audited clean otherwise: `lang`, one `h1`, landmarks, alt on every
  image, no unnamed controls, visible focus rings. Two heading-order jumps
  remain (h1→h3, h2→h4) and are a design decision, not a patch.
...
- **Content the site is publishing that it probably should not be.** The
  contact page's phone number is `+1 (415) 555-0134`, inside the range reserved
  for fiction; its email is on `auxtech.studio` rather than the site's own
  domain (a decorative mockup in `device-frames.tsx` shows that domain too);
  and it claims "50+ senior designers and engineers across nine time zones"
  and three offices while the homepage sells "one senior team" and the tagline
  is "A studio, not a factory." All CMS-editable.
- **A blog post about the CMS this project migrated away from.** "Why we
  rebuilt Auxtech on Payload" is live; the rebrand's find-replace changed the
  title but not the subject. Its slug and one other still say `northline`;
  changing them breaks existing links unless redirects go in. The stock
  "Hello world!" post was set to draft on 2026-09-22, and the grammar the
  find-replace broke ("a Auxtech") is fixed.
- **22 `learning` docs have no page of their own.** They render as list entries
  on the four category pages, which is coherent, but they are editable
  documents with no detail route. Either build one and put `learning` back in
  the sitemap generator, or leave them as list content deliberately.
- `header` and `site_settings` globals are read by nothing; seeding them would
  give an editor fields with no effect. Wiring `site_settings` into the head
  would put a CMS fetch on every page's critical path, which is why it has not
  been done for a title that changes once a year.
...
edit** against WP whenever CMS wiring changed. After anything touching routing
or the sitemap, **crawl every sitemap URL** and expect zero non-200 — that is
what caught 22 advertised URLs that had never resolved. The CMS layer fails soft by
...
- PowerShell here-strings (`@'...'@`) are a parse error in the Bash tool; use a
  message file for multi-line commit messages. Backticks in a double-quoted
  `git commit -m` run as command substitution and silently empty those words.
- **A headless measurement lies in one direction: too pessimistic.** LCP read
  9.6s in a hidden browser tab and 0.45s fronted; a screenshot taken in the
  same batch as a scroll caught frame zero of a 1.1s count-up and looked like
  four broken zeros; `.focus()` with the pane unfocused reports no focus ring
  because `:focus` cannot match when the document is not focused. Front the
  pane, let it settle, and re-measure before believing any of it.
- **`extract: 1` means nothing on its own.** A cPanel upload can return
  `succeeded: 0` and the extract that follows still reports success, leaving
  the previous build serving. Assert on the upload's own `succeeded`, then
  confirm the change is actually live.
- **Crawling the site fast gets curl throttled.** Both hosts start refusing on
  443 with `000` while the cPanel API still answers 200 and a real browser
  loads the site fine. It is a rate limit, not an outage and not an IP ban —
  check a browser before believing the site is down.
