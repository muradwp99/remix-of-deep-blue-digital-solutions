...
export const Route = createFileRoute("/privacy")({
  // LivePress: editable via the `privacy` Site Page doc, fail-soft to copy below.
  loader: async (): Promise<{ doc: SitePageDoc }> => {
    const doc = await cmsFindOne<Record<string, unknown>>("sitepages", "privacy");
    return { doc };
  },
  head: ({ loaderData }) => {
...
function Page() {
  ...
  return (
    <LegalPage
      title={pageStr(d, "legal_title", "Privacy Policy")}
      intro={pageStr(
        d,
        "legal_intro",
        "We collect as little as we can, tell you exactly what we do with it, and never sell it. This policy explains the details in plain language.",
      )}
