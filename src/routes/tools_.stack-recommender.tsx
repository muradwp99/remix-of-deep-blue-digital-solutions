...
export const Route = createFileRoute("/tools_/stack-recommender")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        { title: tool?.metaTitle ?? "Tech Stack Recommender — Auxtech" },
        { name: "description", content: tool?.metaDesc ?? "" },
        { property: "og:title", content: tool?.metaTitle ?? "Tech Stack Recommender — Auxtech" },
              ...
              >
                ...
                <label className="block">
                  ...
                  <input
                    ...
                    placeholder="Two Python devs, one designer"
                    className={`mt-2 ${inputCls}`}
                  />
                </label>
                <label className="block">
              ...
              <div aria-live="polite">
                ...
                {result && (
                  <ResultPanel
                    footnote={
                      result.ai
                        ? "Recommended by AI with our engineering rubric."
                        : "Our house-default stack; the AI take is briefly unavailable."
                    }
                  >
                ...
                {!running && !result && (
                  <div className="grid h-full min-h-64 place-items-center rounded-3xl border border-dashed border-white/15 p-8 text-center">
                    <p className="max-w-xs text-sm leading-relaxed text-white/45">
                      One paragraph about your product gets you a five-layer stack with reasons, not
                      buzzwords.
                    </p>
                  </div>
                )}
