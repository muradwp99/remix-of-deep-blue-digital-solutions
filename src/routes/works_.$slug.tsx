...
export const Route = createFileRoute("/works_/$slug")({
  head: ({ params }) => {
    ...
    const title = study
      ? `${study.name} — Case Study — Northline Studio`
      : "Case Study — Northline Studio";
    const description = study
      ? `${study.name} (${study.industry}, ${study.year}): ${study.summary}`
...
function CaseStudyPage() {
  ...
  return (
    <SiteShell theme={pageThemes["works"]}>
      {/* ---- Hero (light editorial, big project image) ---- */}
      <section className="block-light">
        <div className="container-page py-20 md:py-28">
          <p className="text-xs uppercase tracking-[0.28em] text-gold" data-reveal>
            {study.industry} · {study.tag} · {study.year}
          </p>
          <h1
      ...
      {/* ---- Supporting imagery (clean on white) ---- */}
      <section className="block-light">
      ...
      {/* ---- Testimonial pull-quote (one dramatic deep band) ---- */}
      {study.testimonial && (
