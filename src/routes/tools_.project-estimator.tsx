...
export const Route = createFileRoute("/tools_/project-estimator")({
  head: () => ({
    meta: [
      { title: "Project Estimator — Auxtech" },
      { name: "description", content: "Scope your build in two minutes: budget range, timeline, and phase plan, before any sales call." },
      { property: "og:title", content: "Project Estimator — Auxtech" },
      { property: "og:description", content: "Budget range, timeline, and phase plan in two minutes." },
    ],
  }),
  component: Page,
});
        ...
        <div className="container-page py-20 md:py-24">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-5 font-display text-5xl font-semibold leading-[0.98] md:text-6xl" data-reveal>
              Scope it before the <span className="text-gold">sales call</span>.
            </h1>
          ...
          <div className="mt-12 grid max-w-5xl gap-8 lg:grid-cols-[1fr_1.15fr]" data-reveal>
            <div className="space-y-7 rounded-3xl border border-white/10 bg-white/[0.03] p-6 md:p-8">
              <div>
                <p className="text-xs uppercase tracking-[0.16em] text-white/60">What are you building?</p>
              ...
              <div>
                <div className="flex items-baseline justify-between">
                  <p className="text-xs uppercase tracking-[0.16em] text-white/60">Pages / screens</p>
              ...
              <div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {FEATURES.map((f) => (
                    <button key={f.id} type="button" onClick={() => toggle(f.id)} className={chip(features.includes(f.id))}>
                      {f.label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setUrgency("standard")} className={chip(urgency === "standard")}>
                    Standard pace
                  </button>
                  <button type="button" onClick={() => setUrgency("fast")} className={chip(urgency === "fast")}>
                    As fast as possible
                  </button>
                </div>
              </div>
            ...
            <div aria-live="polite">
              ...
              {result && (
                <ResultPanel footnote={result.ai ? "Phase math from 200+ shipped projects, brief written by AI." : "Phase math from 200+ shipped projects."}>
                  <div className="flex flex-wrap items-end justify-between gap-6">
                    <div>
                      <p className="text-xs uppercase tracking-[0.16em] text-white/55">Budget range</p>
                      <p className="mt-2 font-display text-4xl font-semibold tabular-nums text-gold md:text-5xl">
                        {money(result.low)} <span className="text-white/40">to</span> {money(result.high)}
                      </p>
                    </div>
                    <div>
                      ...
                      <p className="mt-2 font-display text-4xl font-semibold tabular-nums md:text-5xl">
                        {result.weeks}<span className="text-lg text-white/45"> weeks</span>
                      </p>
                    </div>
                  </div>
                    ...
                    {result.phases.map((p) => (
                      <div key={p.name} className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                        ...
                        <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/[0.06]">
                          <div className="h-full rounded-full bg-gold/70" style={{ width: `${(p.weeks / result.weeks) * 100}%` }} />
                        </div>
              ...
              {!running && !result && (
                <div className="grid h-full min-h-64 place-items-center rounded-3xl border border-dashed border-white/15 p-8 text-center">
                  <p className="max-w-xs text-sm leading-relaxed text-white/45">
                    Pick your shape on the left. You get a budget range, a week count, and the four phases mapped out.
                  </p>
                </div>
              )}
            </div>
