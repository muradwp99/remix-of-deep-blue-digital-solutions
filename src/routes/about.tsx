...
export const Route = createFileRoute("/about")({
  // LivePress: the whole page is editable via the `about` Site Page doc;
  // every field falls back to the built-in copy below.
  loader: async (): Promise<{ doc: SitePageDoc }> => {
    const doc = await cmsFindOne<Record<string, unknown>>("sitepages", "about");
    return { doc };
  },
  head: ({ loaderData }) => {
    ...
    const description = pageStr(
      d ?? null,
      "meta_description",
      "A boutique software studio of designers, engineers and strategists.",
    );
    return {
      meta: [
        { title },
...
function AboutPage() {
  ...
  const values = pageRows(d, "values", valuesFallback);
  const wired = pageLines(d, "wired_chips", wiredFallback);
  const team = pageRows(d, "team_members", teamFallback);
  ...
  return (
    <SiteShell theme={pageThemes["about"]}>
        ...
        <div className="container-page grid items-end gap-12 py-24 md:grid-cols-[1.15fr_0.85fr] md:py-32">
          <div>
            ...
            <h1
              ...
              data-reveal
            >
              {s("hero_title", "A studio built for")}{" "}
              <span className="text-gold">{s("hero_title_em", "craft")}</span>.
            </h1>
        ...
        <div className="container-page grid items-start gap-16 border-t border-border py-24 md:grid-cols-2">
          <div data-slide="left" className="flex flex-col gap-8">
            <h2 className="font-display text-4xl font-semibold leading-tight md:text-6xl">
              {s("story_heading", "Founded in 2014 by two friends who wanted to build better software.")}
            </h2>
            <img
              src={s(
                "story_img",
          ...
          <div className="space-y-5 text-lg leading-relaxed text-muted-foreground" data-slide="right">
            <p>
              {s(
                "story_p1",
                "We started as a two-person studio taking on projects other agencies wouldn't: the ambitious ones, the technically hard ones, the ones with a clear point of view.",
              )}
            </p>
            <p>
              {s(
            ...
            <p>
              {s(
                "story_p3",
                "We work with early-stage founders, growth-stage product teams and household brands. We are equally happy to build a landing page, an ecommerce platform, or a full product from scratch — as long as the ambition matches ours.",
              )}
            </p>
          </div>
        </div>
      ...
      <section className="block-light">
        <div className="container-page py-24">
          <h2 className="max-w-2xl font-display text-4xl font-semibold leading-tight md:text-6xl" data-split>
            {s("principles_heading", "How we work.")}
          </h2>
            ...
            {wired.map((label, i) => (
              <span
                ...
                className={`rounded-full px-5 py-2.5 text-sm font-medium glass ${
                  i % 3 === 0 ? "border border-gold/40 text-gold" : "text-foreground/85"
                }`}
              >
                {label}
              </span>
        ...
        <div className="container-page py-24">
          <div className="flex flex-wrap items-end justify-between gap-6">
            <h2 className="font-display text-4xl font-semibold leading-tight md:text-6xl" data-split>
              {s("team_heading", "Senior. Every seat.")}
            </h2>
      ...
      <CTABand
        eyebrow={s("cta_eyebrow", "Work with us")}
        title={s("cta_title", "Sound like your kind of studio?")}
        subtitle={s(
          "cta_subtitle",
          "Tell us what you're building. A founding partner reads every brief and replies within one business day.",
        )}
        primary={{ label: s("cta_primary_label", "Start a Project"), to: "/contact" }}
        secondary={{ label: s("cta_secondary_label", "Meet the Leadership"), to: "/leadership" }}
      />
    </SiteShell>
  );
}
